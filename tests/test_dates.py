...
def test_gen_date_5(item):
    ...
    min_date = datetime.date.today() - datetime.timedelta(365 * MIN_YEARS)
...
def test_gen_date_6(item):
    ...
    max_date = datetime.date.today() + datetime.timedelta(365 * MAX_YEARS)
...
def test_gen_date_10():
    """Create a date with non-Date arguments."""
    with pytest.raises(ValueError):
        gen_date(min_date=1, max_date=1)
